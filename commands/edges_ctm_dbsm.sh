...
# MODEL_FLAGS="--data_name=edges2handbags --microbatch=64 --batch_size=64 --global_batch_size=64
...
# # Comb:
# MODEL_FLAGS="--data_name=edges2handbags --microbatch=128 --batch_size=128 --global_batch_size=128
# --class_cond=False --eval_interval=1000 --save_interval=5000 --num_classes=10 
...
# CM:
MODEL_FLAGS="--data_name=edges2handbags --microbatch=64 --batch_size=64 --global_batch_size=64
--class_cond=False --eval_interval=10 --save_interval=5000 --num_classes=10 
...
--qpart_loss=False --do_xT_precond=True --use_x0_as_denoised_in_solver=True
--ema_rate=0.99993 --start_ema=0. --num_heun_step=1 --churn_step_ratio=0
--is_I2I=True --gamma=1. --lr=0.0002 --total_training_steps=400000
--compute_ema_fids=True --sample_s_strategy=sigma_s_is_zero --schedule_sampler=ict
--target_ema_mode=fixed --scale_mode=ict_exp --start_scales=10 --end_scales=1280
--traditional_ctm=False --apply_adaptive_weight=False
...
# DATASET_NAME="edges2handbags_qPartloss"
# DATASET_NAME="edges2handbags"
DATASET_NAME="edges2handbags"
EXP_DATE=0910
SAMPLER=contri_ddpm_pp_cm
SOLVER=no_solver
...
--ref_path=/root/data/FID_stats/edges2handbags_ref_64_data.npz 
...
# CKPT_FLAGS="--out_dir /root/code/results/0503/edges2handbags --data_dir=/root/data/edges2handbags/"
